...
(account `C5FUHLGUOBGJRA2PNDVTXY2MOU`, environment ID hardcoded in
`secrets.go` — not itself a secret, just an identifier). `secrets.go` reads
it directly with `github.com/1password/onepassword-sdk-go`'s
`WithDesktopAppIntegration`, which pops a native Touch ID/password prompt in
the 1Password app on first use. **There is no local `.env` file** — an
